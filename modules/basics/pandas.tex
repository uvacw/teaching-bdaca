...
\textbf{Copy vs. reference:}  
The key difference is between creating a \textit{copy} and referring to the \textit{same object} in memory.

\begin{minted}[fontsize=\scriptsize, bgcolor=gray!5]{python}
# Wrong way — chained indexing creates a copy
df[df['gender']==0]['radio'] = 0
\end{minted}

\begin{block}{Warning output}
\begin{minted}[fontsize=\scriptsize, bgcolor=yellow!5]{text}
...
\begin{minted}[fontsize=\scriptsize, bgcolor=gray!5]{python}
# Correct way — modifies the original DataFrame
df.loc[df['gender']==0, 'radio'] = 0
\end{minted}
...
\begin{frame}{\texttt{.agg()}}
\begin{itemize}[<+->]
  \item Very useful after a \texttt{.groupby()}.
  \item Takes a function (or list of functions) as argument:
  \begin{itemize}
    \item Single function: \\
      \texttt{df2 = df.groupby('region').agg('sum')}
    \item Multiple functions: \\
      \texttt{df2 = df.groupby('region').agg(['sum', 'mean'])}
  \end{itemize}
  \item $\rightarrow$ You could use \texttt{.describe()}, but \texttt{.agg()} is more flexible.
\end{itemize}
\end{frame}
...
df.groupby("gender")['internet'].agg(['mean', 'std'])
\end{minted}
\end{frame}
